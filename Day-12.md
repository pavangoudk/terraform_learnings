...
locals {
  # Mapping from environment name to machine type
  env_machine_map = {
    Dev     = "Standard_B2s"
  ...
  # Use lookup for mapping, fallback to default if not found
  machine_type = lookup(local.env_machine_map, var.environment, "Standard_B2s")
}
...
# Validate a required file path exists
variable "tfvars_file_path" {
  description = "Path to the required .tfvars file."
  type        = string

  validation {
    condition     = fileexists(var.tfvars_file_path)
    error_message = "The specified .tfvars file does not exist at the given path."
  }
}

# Combine two lists of locations from different sources
variable "primary_locations" {
  type    = list(string)
...
locals {
  ...
  all_unique_locations = toset(concat(var.primary_locations, var.additional_locations))
}

output "unique_locations" {
  value = local.all_unique_locations
}
